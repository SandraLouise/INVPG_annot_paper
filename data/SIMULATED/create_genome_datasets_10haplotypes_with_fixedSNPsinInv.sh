...
## It creates 9 other haplotypes with each a random sample of 50% of the INVs, all the SNPs inside the selected INVs and outside all INV a random sample of 50 % of the SNPs.
## It formats the corresponding fasta files and input files as needed for the 4 pangenome graph pipelines
##
## Note: HERE SNP alleles inside INVs are all (or 500% of thme with ALLFIXED=1) fixed with the inversions. The SNP divergence inside INVs is 2*DIV, while outside all inversions, it is DIV
...
DIV=$2   #0.1 for 0,1% div ; 1 for 1%div  // cannot be 0
...
ALLFIXED=0  # if 0 50% of the SNPs inside INVs are fixed with the INVs ; if 1, all the SNPs inside INV are fixed, so with a level of divergence = 2*DIV 
#ex : ./create_genome_datasets_10haplotypes_with_fixedSNPsinInv.sh chr21.fa 0.1 chr21_sim_100inv.vcf chm13 tenhaplofixed genomes_10hap_fixed 10 

# Fixed input & env
nHAP=9
invPCT=50
# snpPCT=50  fixed at 50% in script 02b_...
...
# 2. Simulate 9 haplotypes with SNPs and 50% INV
# --------------------------------------------------
...
if [ "$DIV" != "0" ]; then
python3 $BINDIR/02b_simulate_snps_and_inversions_in_1_chrom.py $REF $SNP $INV $outfinal $invPCT $header $ALLFIXED $indivSEED
# 3 files: $outfinal.fa $outfinal.vcf $outfinal_snps.vcf
