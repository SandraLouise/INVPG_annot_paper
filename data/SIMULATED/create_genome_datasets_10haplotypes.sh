...
## It creates 9 other haplotypes with each random samples of 50% of the SNPs and 50% of the inv, and formats the corresponding fasta files and input files as needed for the 4 pangenome graph pipelines
##
...
#ex : ./create_genome_datasets_10haplotypes.sh chr21.fa 0.1 chr21_sim_100inv.vcf chm13 tenhaplo genomes_10hap 10 
...
CACTUS_file="cactus_input_10hap_div$DIV.txt"
...
# 2. Simulate 9 haplotypes with 50% SNPs and 50% INV
# --------------------------------------------------
...
if [ "$DIV" != "0" ]; then
python3 $BINDIR/03a_simulate_inversions_in_1_synthetic_chrom.py $outsnp $INV $outfinal $invPCT $header $indivSEED
# 2 files: $outfinal.fa $outfinal.vcf

# cleaning
rm -f $outsnp
else
python3 $BINDIR/03a_simulate_inversions_in_1_synthetic_chrom.py $REF $INV $outfinal $invPCT $header $indivSEED
# 2 files: $outfinal.fa $outfinal.vcf
fi
...
MINIGRAPH_file="minigraph_input_10hap_div$DIV.txt"
